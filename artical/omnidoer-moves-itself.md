...
## 第七件事：先用 IP 验证，而不是直接改 DNS

用户没有急着按下 DNS 开关。

他问：

> 先用 IP 是否可以，IP 可用再切 DNS？

这是正确的顺序。

OmniDoer 因此没有动 DNS，而是用 `152.32.233.23` 做公网直测：

- SSH、SMTP、IMAP、HTTPS、SOCKS5、Flask、Control Client 等 TCP 端口都能从旧机连到新机。
- `http://152.32.233.23:3552/` 直接返回 Flask 页面。
- `https://152.32.233.23:8787/` 在忽略证书域名不匹配后返回 Control Client 页面。
- `https://482692.xyz/` 强制解析到 `152.32.233.23` 后返回 200。
- `https://482692.xyz:8787/` 强制解析到 `152.32.233.23` 后返回 200。
- 新机本地邮件认证成功，SOCKS5 代理可以访问外网。

也有一个预期现象：

`http://152.32.233.23/` 裸 IP 返回 404，因为 nginx 的正式站点按 `482692.xyz` 的 Host 和 TLS 证书工作。  
这不是失败，而是虚拟主机配置的正常结果。

不过这个结果对用户预检不够友好。

于是 OmniDoer 又补了一步：在新机 nginx 里增加一个默认 HTTP server，专门用于裸 IP 预检，把 `http://152.32.233.23/` 也代理到 Flask。

这一步完成后，裸 IP 首页也返回 200。
用户可以直接打开 `http://152.32.233.23/` 看主站状态。
Control Client 仍然可以用 `https://152.32.233.23:8787/` 预检，只是因为证书签给 `482692.xyz`，浏览器会出现证书域名提示。
...
> 如何在新的服务器 resume 这个当前会话？

OmniDoer 给出了当前 thread 的恢复命令：

```text
omnidoer console resume 019e7d42-ecc4-70f2-b8bb-34517661f922
```

但这还不够。

旧服务器之所以能在网络波动、手机浏览器刷新、终端断开时尽量保住对话，是因为 root 登录后会自动进入一个固定的 tmux 会话：`main`。

新服务器一开始只有 tmux 程序，没有这个自动附着规则。

于是 OmniDoer 把同样的稳定会话机制迁过去：

- 新机 root SSH 交互登录会自动 attach 到 `main`。
- 如果 `main` 不存在，就自动创建。
- 非交互 SSH、scp、rsync 和后台服务不受影响。
- `.tmux.conf` 保持鼠标可用，并用状态栏显示当前在 `main` 会话中。

验证时，一个新的伪终端 SSH 登录进入了 tmux，随后打印出：

```text
AUTO_TMUX_SESSION=main WINDOW=bash
```

这意味着用户在新服务器上的终端会话也开始具备断线后可恢复的基础。
